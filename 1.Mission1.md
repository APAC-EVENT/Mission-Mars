...
오른쪽 아래의 화살표를 눌러 2번째 페이지로 이동하여 Data Source URL에 [](http://azuremlsamples.azureml.net/templatedata/Text - Sentiment Stopwords.tsv)를 입력하고 Data format을 TSV로 변경합니다. (*다른 모듈에 연결하지 않고 다음 스텝으로 넘어가세요)
